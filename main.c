...
#define problemsFilePath "Problems.bin"
...
int checkTech(int techId, int techProdId) {
    ...
    while (!feof(TechnicianFile))
    {
        ...
        if ((techId == tech.tech_id) && (techProdId == tech.productId))
        {
            ...
            printf("----------------------------------\n");
            fclose(TechnicianFile);    return 1;
        }
    }
...
{
    printf("\033[1;33m");
    userFile=fopen( problemsFilePath, "rb");
    if (userFile == NULL) {
        printf("Error! opening userfile to read\n");
        return 0;
    }

    ProblemsFile=fopen( problemsFilePath, "rb");
...
int addProblems()
{
    ...
    ProblemsFile=fopen( problemsFilePath, "ab+");
    ...
    scanf("%d", &problem.userId);
    char c2 = getchar();


    printf("enter product id: \n");
    scanf("%d", &problem.prodid);
    char c4 = getchar();
    ...
    case 2:
        ...
        scanf("%d", &problem.type);
        char c1 = getchar();
        printf("Please enter problem time\n");
        scanf("%lf", &problem.time);
        char c3 = getchar();
        break;
    }
    problem.care = 0;
...
int modifyProblemStatus() {
    ...
    if (ch == 'n')
    {
        ...
        if (ch == 'y') {
            ...
            scanf("%d", &type);//  מחזירה אותו למסך שיבוץ
            problem.type = type;
            printf("type updated.....\n");
        }
        else
